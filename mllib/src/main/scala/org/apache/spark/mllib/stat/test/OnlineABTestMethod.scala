...
sealed trait OnlineABTestMethod extends Serializable {
  ...
  protected implicit def toApacheCommonsStats(
      summaryStats: MultivariateOnlineSummarizer): StatisticalSummaryValues = {
    require(summaryStats.numDim == 1)
    new StatisticalSummaryValues(
      ...
      summaryStats.max(0),
      summaryStats.min(0),
      summaryStats.mean(0) * summaryStats.count
    )
  }
}
...
private[stat] object WelchTTest extends OnlineABTestMethod with Logging {
  ...
  private def test(
      statsA: MultivariateOnlineSummarizer,
      statsB: MultivariateOnlineSummarizer): OnlineABTestResult = {
