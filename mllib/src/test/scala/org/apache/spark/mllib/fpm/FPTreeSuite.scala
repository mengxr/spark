...
class FPTreeSuite extends FunSuite with MLlibTestSparkContext {
  ...
  test("add transaction") {
    ...
    assert(child.children(2).item.equals(2))
    assert(child.children(3).item.equals(3))
