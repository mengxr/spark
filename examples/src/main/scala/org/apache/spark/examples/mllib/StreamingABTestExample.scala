...
import org.apache.spark.mllib.stat.test.{TTester, TTestSummaryStats, StreamingABTest, TTestResult}
 ...
 * the t-test are continually updated as data is provided. The test performed is Welch's
 * two-sample t-test. The program will terminate if the p-value ever drops below 0.01.
 *
 * Usage:
...
object StreamingABTestExample {
  def printUsageAndQuit(): Unit = {
    System.err.println(
      "Usage: StreamingABTestExample <testResultsDir> <batchDuration> <windowLength>" +
          " <peacePeriod> <maxBatches>")
    System.exit(1)
  }
  ...
  def main(args: Array[String]) = {
    ...
    val test = new StreamingABTest(TTestSummaryStats(), TTester.performTest,
      windowLength = Seconds(windowLength), peacePeriod = peacePeriod)
